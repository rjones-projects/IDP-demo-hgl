...
module "bastion_vm" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/bastion_vm?ref=main"

  project_id                     = var.project_id
  bastion_vm                     = var.bastion_vm
  bastion_vm_default             = var.bastion_vm_default
}
...
module "cloud_run" {
  ...
  managed_revision               = var.managed_revision
  name                           = var.name
  ...
  type                           = var.type
  volumes                        = var.volumes
  workerpool_config              = var.workerpool_config
...
module "vf_security_policy" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/vf_security_policy?ref=main"

  project_id                     = var.project_id
  vf_security_policy             = var.vf_security_policy
  vf_security_policy_default     = var.vf_security_policy_default
}
...
module "cloud_sql" {
  ...
  cloud_sql_default              = var.cloud_sql_default
}

module "bigquery" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/bigquery?ref=main"

  project_id                     = var.project_id
  region                         = var.region
  bigquery                       = var.bigquery
  dataset_default                = var.dataset_default
}

module "bigquery_table" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/bigquery_table?ref=main"

  project_id                     = var.project_id
  region                         = var.region
  bigquery_table                 = var.bigquery_table
  table_default                  = var.table_default
}

module "composer_environment" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/composer_environment?ref=main"

  project_id                     = var.project_id
  region                         = var.region
  composer_environment           = var.composer_environment
  composer_environment_default   = var.composer_environment_default
}

module "cloud_build" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/cloud_build?ref=main"

  project_id                     = var.project_id
  region                         = var.region
  cloud_build                    = var.cloud_build
  cloud_build_default            = var.cloud_build_default
}

module "cloud_build_private_worker_pool" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/cloud_build_private_worker_pool?ref=main"

  project_id                     = var.project_id
  region                         = var.region
  cloud_build_private_worker_pool = var.cloud_build_private_worker_pool
  cloud_build_private_worker_pool_default = var.cloud_build_private_worker_pool_default
}

module "vpc_connector" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/vpc_connector?ref=main"
...
module "workload_identity" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/workload_identity?ref=main"
...
module "pubsub" {
  source = "github.com/rjones-projects/gcp_terraform-modules//terraform/modules/pubsub?ref=main"

  project_id                     = var.project_id
  region                         = var.region
  pubsub                         = var.pubsub
  topic_default                  = var.topic_default
  subscription_default           = var.subscription_default
}
